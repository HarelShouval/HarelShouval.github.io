 ...
 // begin display frustrations
  if (counter>0){ // only make plot 3 for counter >0
    ...
    //push();
     ...
     text('My Frustrations', (width/2-250)*0.66,10);
     fill('Red');
    ...
    line(plotx_buffer+(counter)*box_w,plot3_height/3-(frustrations_av[counter-1]-frustrations_av[0])*plot3_height/(n*0.5),x_pad+(counter+1)*box_w,plot3_height/3-(frustrations_av[counter]-frustrations_av[0])*plot3_height/(n*0.5));
...
function control_panel(){
    ...
    if (collect_choice==1)
      ...
      {push();
       ...
       buttonReset.style('background-color','Gray');
       pop();
      }
